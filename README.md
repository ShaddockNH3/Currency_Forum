...
## ✨ 项目预期的核心功能 (Core Features)

本项目旨在构建一个功能完善的“迷你货币论坛”，用户不仅可以在这里浏览和讨论与货币相关的新闻资讯，还可以体验一套完整的模拟金融交易系统。
...
* **个人主页**: 每位用户都将拥有一个独立的个人主页，用于展示其个性签名、个人简介，以及所有已发布过的文章列表（支持分页）。

### 📰 论坛与内容模块 (Forum & Content Module)

* **文章管理**: 用户可以发布、编辑、删除自己的文章。管理员则拥有管理所有文章的权限。
* **评论系统**: 用户可以对文章进行评论和回复，形成自由的讨论氛围。
* **内容互动**: 用户可以对自己喜欢的文章进行“点赞”。

### 💸 核心金融与交易系统 (Core Financial & Transaction System)

*这是一个完全模拟的、不涉及真实货币的内部交易系统，旨在提供有趣和有教育意义的体验。*

* **多币种钱包**: 每位用户在注册后，都会自动创建一个可以持有多种货币（如 USD, JPY, EUR）的个人钱包。
* **资金操作 (模拟)**: 用户可以向自己的钱包进行 **充值 (SaveMoney)** 和 **提现 (WithdrawMoney)** 操作。
* **货币兑换 (Exchange)**: 用户可以根据系统内设定的（或未来对接的实时）汇率，自由地在不同货币之间进行兑换。
* **用户间交易 (P2P Trading)**: 用户之间可以发起和接受交易请求，实现平台内资产的自由流转。
* **交易记录**: 所有的资金变动（充值、提现、兑换、交易）都会被记录下来，形成清晰的个人账单。

### 🧮 工具与其他 (Tools & Others)

* **汇率查看器**: 提供一个公开的页面或工具，用于查看当前系统内的所有模拟汇率，并可以进行换算。
* **后台管理**: 为管理员提供必要的接口，以管理全站用户、文章及汇率等信息。
